...
class _HomePage2WidgetState extends State<HomePage2Widget>
  ...
  Widget build(BuildContext context) {
    ...
    String used = (((map["ram"]["total"]).split(" "))[0]);

    double nominator = double.parse(used);
    String total = (((map["ram"]["total"]).split(" "))[0]);
    double denominator = double.parse(total);

    final String uid = "bCy9k93Bmdg56xhgm4mgJhtyYB92";
    return StreamBuilder<DocumentSnapshot>(
      stream: FirebaseFirestore.instance.collection('users').doc(uid).snapshots(),
      builder: (BuildContext context, AsyncSnapshot<DocumentSnapshot> snapshot) {
        Map<String, dynamic> userData = snapshot.data!.data() as Map<String, dynamic>;
        String ramfree = userData['ramfree'];
        String ramused = userData['ramused'];
        String ramtotal = userData['ramtotal'];
        return Scaffold(
